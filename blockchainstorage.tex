...
\tikzset{%
	...
	block/.style = {draw, thick, shape=rectangle, inner sep=0.5cm, align=left},
	list/.style = {draw, thick, shape=paper, inner sep=0.5cm, align=left}
}
...
\begin{tikzpicture}
	...
	\node (null) [draw=none, minimum height=3.3cm, minimum width=3.3cm] {\Large \texttt{null}};

	\node (block0) [block, right=1.5cm of null] {
		\begin{minipage}[t][2.3cm]{2.3cm}
		\texttt{Add: A\\ Add: B\\ Add: C\\ Sig: \{\}}
		\end{minipage}
	};

	\node (block1) [block, right=1.5cm of block0] {
		\begin{minipage}[t][2.3cm]{2.3cm}
	...
	\draw[->, dashed, thick]  (group) .. controls ($ (block1) + (3,0.5) $) and
			($ (list1) + (3,-0.5) $) .. ($ (list1) + (0,-0.85) $);
	\draw[->, dashed, thick] ($ (list1) + (0,-1) $) -- ($ (list1) + (2,-1) $)
