...
\begin{tikzpicture}
	...
	\draw[brace, decoration={mirror}]
			...
			node[midway, xshift=-0.7cm] {\tiny \#\footnotesize Tx};

	% The structure of a transaction header
	\node (field0) [firstfield, right=3cm of chunk0] {\texttt{\small Type}};
	...
	\node (ptrsize) [above right=0.1cm and -0.1cm of dataend, align=left]
			{\footnotesize Pointer + Size};

	% Descriptions
	\node [above=0.1cm of chunk0] {\footnotesize Block};
	\node [above=0.1cm of field1] {\footnotesize Transaction Header};

	% Draw appropriate connections
	\coordinate[right=of chunk2]  (im0); % Transaction
	\draw[thick] (chunk2) -| (im0) |- (field0);
	\draw[thick] (chunk5) -- (data0); % Data
	\coordinate[below=of field2]  (im1); % Transaction -> data
	\draw[->, thick] (field2) |- (im1) -| (data3.north west);
	\draw[brace] ($ (data3.north west) + (0.1,0) $) -- ($(dataend.north west) + (-0.1,0) $) % tx data
			node[midway, yshift=0.6cm] {\footnotesize Tx data};
